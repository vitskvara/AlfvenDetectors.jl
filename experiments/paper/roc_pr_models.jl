...
common_f = "/compass/home/skvara/alfven/experiments/roc_prc_eval_data.jld2"
common_data = load(common_f)
...
if !isfile(common_f)
    ...
    train_labeled = common_data["train_labeled"]
    test = common_data["test"]
    train_unlabeled = (cat(test[1][:,:,:,1:1], train_labeled[1], dims=ndims(train_labeled[1])),
        vcat(zeros(size(test[1][:,:,:,1:1], ndims(test[1][:,:,:,1:1]))), train_labeled[2]))
end
...
prec_50 = EvalCurves.precision_at_k(as, _labels, 50)
...
CSV.write("/compass/home/skvara/alfven/experiments/conv/roc_prc_two_stage_commons.csv", df)
